...
public class AppiumFunctions extends TwcAndroidBaseTest{
	...
	public static void startAppium() throws InterruptedException {
		...
		/*String[] openAppium_str ={"/bin/bash", "-c", "-appium"};
 try {
			Runtime.getRuntime().exec(openAppium_str);
			Thread.sleep(5000);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
public static void stopAppium() throws InterruptedException {
 ...
 try {
	...
	public static void AppiumServerStart() throws InterruptedException{
		...
		//command.addArgument("/Applications/Appium2.app/Contents/Resources/app/node_modules/appium/bin//appium.js");
				CommandLine command = new CommandLine("/usr/local/bin/node");
		command.addArgument("/Applications/AppiumServerGUI.app/Contents/Resources/app/node_modules/appium/build/lib/appium.js");
...
//		command.addArgument("--relaxedSecurityEnabled", true);
//		command.addArgument("--allowDelayAdb", true);
//		command.addArgument("--allowInsecure", true);
		command.addArgument("error");
		
//		//String[] command ={"/bin/bash","-c","appium"};
//		 // --address 127.0.0.1 --port 4724 --session-override
//		String[] command = {"/bin/bash","-c","appium --address 127.0.0.1 --port 4724 --session-override"};
...
//      		try {  		
    	...
    	//	ReadExcelValues.excelValues("Smoke", "Appium");
    		DeviceStatus device_status = new DeviceStatus();
 			int Cap = device_status.Device_Status();
    		String[][] capabilitydata = read_excel_data.exceldataread("Appium");
    		CommandLine command = new CommandLine(capabilitydata[1][Cap]);
    		command.addArgument(capabilitydata[2][Cap], false);
    		command.addArgument("--address", false);
    		command.addArgument(capabilitydata[3][Cap]);
    		command.addArgument("--port", false);
    		command.addArgument(capabilitydata[4][Cap]);
    	...
    	public static void stopAppiumServer() throws IOException, Exception {
     	...
     	public static void clickOnHomeTab() throws Exception {
     		Ad.findElementByAccessibilityId("Home Tab").click();
     		Thread.sleep(10000);
     	}
...
public static void clickONTerms() throws Exception{
	try {
		//new WebDriverWait(Ad, Functions.maxTimeout).until(ExpectedConditions.elementToBeClickable(Ad.findElementById("com.weather.Weather:id/ok_button")));
...
public static void scrollforclickngIunderstand() throws Exception{
	try {
...
try {
	new WebDriverWait(Ad, Functions.maxTimeout).until(ExpectedConditions.elementToBeClickable(Ad.findElementById("com.android.permissioncontroller:id/permission_allow_always_button")));
...
public static void settingProxyEnable(String type,String ipAddress,String portNumber) throws Exception{
	...
	clickRequiredEleemntononSettingsapp("Wi‑Fi");
	Thread.sleep(8000);
	clickRequiredEleemntononSettingsapp("Micros");		
	Thread.sleep(10000);
...
if(i==swipeup) {
	...
	//	Thread.sleep(20000);
		...
		new WebDriverWait(Ad, Functions.maxTimeout).until(ExpectedConditions.elementToBeClickable(Ad.findElementById("android:id/button1")));
		Ad.findElementById("android:id/button1").click();	
	     Ad.isAppInstalled("");
